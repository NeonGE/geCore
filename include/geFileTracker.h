...
namespace geEngineSDK {
  ...
  {
    std::size_t
    operator()(const TrackedFile& file) const noexcept {
      return std::hash<uint32>()(file.m_systemID) ^ std::hash<WString>()(file.m_filePath);
    }
  };

  class FileTracker final : public Module<FileTracker>
  {
   public:
    FileTracker() = default;
    ~FileTracker() = default;
     ...
     */
    void
    addFiles(const StringID systemID, const Vector<Path>& newFiles);
