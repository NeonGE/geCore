...
namespace geEngineSDK {
  ...
  TextureManager::reload(const Path& filePath) {
    if (isLoaded(filePath)) {
      Lock lock(m_mutex);
      m_loadedTextures.erase(StringID(filePath.toString()).id());
    }
  ...
  TextureManager::load(const Path& filePath,
    ...
    if (isLoaded(filePath)) {
      ...
      Lock lock(m_mutex);
      if (pbTestForAlpha) {
        *pbTestForAlpha = m_loadedTextures[fileID.id()]->haveAlpha();
      }
      return m_loadedTextures[fileID.id()];
    }
    ...
    bool bIntTestForAlpha = false;
    auto pTexture = graphMan.loadTextureFromFile(realPath,
    ...
    if (pbTestForAlpha) {
      *pbTestForAlpha = bIntTestForAlpha;
    }

#if USING(FILE_TRACKER)
    auto& fileTracker = g_fileWatcher().instance();
    fileTracker.addFiles({ filePath });
    //fileTracker.restartWatching();
#endif

    pTexture->setPath(filePath);
    pTexture->setCachePath(TextureCachePath);
