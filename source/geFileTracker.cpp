...
namespace geEngineSDK {
  ...
  void
  FileTracker::addFiles(const StringID systemID, const Vector<Path>& newFiles) {
    Path currentDir = FileSystem::getWorkingDirectoryPath();
    {
      ScopedLock<true> lock(m_dataMutex);

      if (m_subscribersCallbacks.find(systemID.id()) == m_subscribersCallbacks.end()) {
        //If the system is not subscribed, we do not track its files
        GE_LOG(kWarning,
          FileTracker,
          "System with ID {} is not subscribed to file changes.", systemID.id());
        return;
      }
      ...
      for (const auto& file : newFiles) {
        ...
        trackedFile.m_systemID = systemID.id();
